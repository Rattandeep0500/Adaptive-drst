...
def main():
    ...
    print()
    print(
        f"DANN Baseline USPS Accuracy: "
        f"{dann_target_accuracy:.2f}%"
    )

    probabilities, target_indices = predict_target(
        feature_extractor,
        classifier,
        target_dataset,
        device,
    ...
    selected_indices, pseudo_labels, confidences = (
    ...
    print()
    print(
        f"Pseudo-label Threshold: 0.90"
    )

    print(
        f"Selected Pseudo-labels: "
        f"{len(selected_indices)}"
    )

    print(
        f"Selection Coverage: "
        f"{100.0 * len(selected_indices) / len(target_dataset):.2f}%"
    )

    if confidences:
        print(
            f"Mean Pseudo-label Confidence: "
            f"{np.mean(confidences):.4f}"
        )
    else:
        print(
            "Mean Pseudo-label Confidence: 0.0000"
        )
    ...
    remaining_indices = [
        ...
        if not selected_mask[int(index)]
    ]
    ...
    confidence_schedule_epochs = 5

    for epoch in range(
        confidence_schedule_epochs
    ):
        source_batches = [
            batch
            for batch in locked_batches
        ...
        source_loss, target_loss, domain_loss = (
            train_pseudo_epoch(
                feature_extractor,
                classifier,
                domain_discriminator,
        ...
        if remaining_indices:
            remaining_x = collect_target_batch(
                target_dataset,
                remaining_indices,
                device,
            )

            remaining_features = feature_extractor(
                remaining_x
            )

            remaining_logits = classifier(
                remaining_features
            )

            remaining_probabilities = F.softmax(
                remaining_logits,
                dim=1,
            ).cpu()

            selected_new, labels_new, confidence_new = (
                select_pseudo_labels(
            ...
            pseudo_targets.extend(
                labels_new
            )

            selected_new_set = set(
                selected_new
            )
            ...
            remaining_indices = [
                ...
                if index not in selected_new_set
            ]
    ...
    print()
    print(
        f"Final DANN Source Accuracy: "
        f"{final_source:.2f}%"
    )

    print(
        f"Final DANN+DRST USPS Accuracy: "
        f"{final_target:.2f}%"
    )
    ...
    print(
        f"Total Pseudo-labels Used: "
        f"{len(pseudo_indices)}"
    )

    print(
        f"Final Pseudo-label Coverage: "
        f"{100.0 * len(pseudo_indices) / len(target_dataset):.2f}%"
    )

    print(
        f"Improvement Over DANN: "
        f"{final_target - dann_target_accuracy:+.2f} pp"
    )
