...
OUTPUT_CHECKPOINT = "checkpoints/dann_ema_soft_centroid_seed42.pt"
HISTORY_PATH = "checkpoints/dann_ema_soft_centroid_seed42.json"
...
def class_centroid_alignment(
    ...
    for class_id in range(10):
        ...
        class_target_features = target_features[target_mask]
        class_target_weights = target_weights[target_mask]

        target_centroid = (
            class_target_features * class_target_weights.unsqueeze(1)
        ).sum(dim=0) / class_target_weights.sum().clamp_min(1e-8)
...
def train_one_epoch(
    ...
    for step in range(steps):
        ...
        if step < len(source_loader):
            xs, ys = next(source_iterator)
            xs = xs.to(DEVICE)
            ys = ys.to(DEVICE)

            source_features = student_feature(xs)
            source_logits = student_classifier(source_features)
            ...
            total_loss = total_loss + source_loss
            source_loss_total += source_loss.item()

        if step < len(target_loader):
            xt, yt, wt = next(target_iterator)
            xt = xt.to(DEVICE)
            yt = yt.to(DEVICE)
            wt = wt.to(DEVICE)

            target_features = student_feature(xt)
            target_logits = student_classifier(target_features)
        ...
        if (
            ...
            centroid_loss = class_centroid_alignment(
                source_features,
                source_labels,
                target_features,
                target_labels,
            ...
            centroid_loss_total += centroid_loss.item()
            centroid_steps += 1
    ...
    return (
        source_loss_total / max(len(source_loader), 1),
        target_loss_total / max(len(target_loader), 1),
        centroid_loss_total / max(centroid_steps, 1),
    )
...
def load_dann_checkpoint(
    ...
    if (
        "feature_extractor" not in checkpoint
        or "classifier" not in checkpoint
        or "domain_discriminator" not in checkpoint
    ):
        raise RuntimeError(
...
def main():
    ...
    print(f"centroid_weight={CENTROID_WEIGHT}")
    print()
    ...
    for epoch in range(1, NUM_PL_EPOCHS + 1):
        ...
        source_loss, target_loss, centroid_loss = train_one_epoch(
            student_feature,
            student_classifier,
            teacher_feature,
            teacher_classifier,
